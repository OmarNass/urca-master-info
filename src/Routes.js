...
const Routes = () => {
    ...
    async function getSlugs(type) {
        try {
            const res = await axios.get('/api/v3/content/' + type + '/?key=cac15f28976f6e0ff9542ef6a6&fields=slug')
            if (type === 'pages')
                setPageSlugs(res.data.pages)
            else 
                setPostSlugs(res.data.posts)
        } catch (err) {
            ...
            <Route exact path="/projets" component={Projects}  />
            {
                pageSlugs.map(slug => (
                    <Route key={slug.slug} exact path={'/' + slug.slug}
                        render={(props) => <Page {...props} type="pages" />}/>
                ))
            }
            {
                postSlugs.map(slug => (
